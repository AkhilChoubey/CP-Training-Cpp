...
void bottomViewHeper(TreeNode *root, int horizontalLevel, int verticalLevel, map<int, pair<int, int>> &levelNodes)
{
    if (root == NULL)
    {
        return;
    ...
    bottomViewHeper(root->left, horizontalLevel - 1, verticalLevel + 1, levelNodes);
    bottomViewHeper(root->right, horizontalLevel + 1, verticalLevel + 1, levelNodes);
}
